...
每路输出独立拥有一组 RUN（绿色）和 ERR（红色）指示灯，共 6 颗 LED。

| 场景 | RUN 灯 | ERR 灯 | 说明 |
|------|--------|--------|------|
| 通道正常输出 | 常亮 | 熄灭 | 该通道已开启且电压/电流正常 |
| 通道关闭（手动） | 熄灭 | 熄灭 | 用户通过按键手动关闭通道 |
| 通道关闭（保护触发） | 熄灭 | 常亮 | 已开启的通道 ERR 灯开启 |
| 急停未松开 | 熄灭 | 全部常亮 | 电压跌至 2V 以下触发整机急停，所有 ERR 灯点亮 |
| 按键：等待二次长按 | 慢闪（2Hz） | 熄灭 | 首次短按后的 5s 等待窗口 |
| 按键：长按计时中 | 流水灯 | 参与流水 | RUN 和 ERR 交替点亮，每步 250ms |
| 按键：等待超时 / 松手取消 | 熄灭 | 熄灭 | 退出操作，恢复空闲 |

蜂鸣器指示：

| 场景 | 蜂鸣器 | 说明 |
|------|--------|------|
| 系统上电初始化 | 三声递进音（1.2k → 2.2k → 2.4kHz，各 300ms） | 上电自检完成 |
| 电压/电流异常预警 | 持续 2.2kHz 鸣响 | 低压预警、过压、过流、突变均触发 |
| 所有通道关闭 | 静音 | 无告警时自动关闭蜂鸣器 |
